# ==============================================================================
# INTEGRATED SENSITIVITY ANALYSIS CONTROLLER & PLOTTER (v50 - Finer Sweeps)
# ==============================================================================
...
database_file     <- sprintf("Morris_Omnibus_rev36_r%d.rds", trajectories)
plot_prefix       <- "rev36_"
...
if (file.exists(database_file) && !force_recalculate) {
  ...
  for (var in names(sweep_targets)) {
    if (var %in% names(base_vars)) {
      st <- sweep_targets[[var]]
      if (!is.null(st$breaks)) {
        # Explicit (possibly non-uniform) chunk edges
        brk <- st$breaks
        ints <- lapply(seq_len(length(brk) - 1), function(i) c(brk[i], brk[i + 1]))
      } else {
        min_b <- st$bounds[1]; max_b <- st$bounds[2]
        n <- st$n_chunks; step <- (max_b - min_b) / n
        ints <- lapply(1:n, function(i) c(min_b + (i - 1) * step, min_b + i * step))
      }
  ...
  for (row in 1:total_scenarios) {
    ...
    for (var in names(chunk_intervals)) {
      ...
      title_details <- c(title_details, sprintf("%s: [%.3g - %.3g]", var, specific_chunk[1], specific_chunk[2]))
    }

    var_names <- names(current_bounds)
    # Log-uniform factors: run the Morris design in log10 space so decade-
    # spanning ranges are sampled evenly (EEs are then per log10-unit).
    design_bounds <- current_bounds
    for (lv in intersect(log_vars, var_names)) design_bounds[[lv]] <- log10(design_bounds[[lv]])
    min_bounds <- sapply(design_bounds, function(x) x[1])
    max_bounds <- sapply(design_bounds, function(x) x[2])
    ...
    X_model <- morris_obj$X
    for (lv in intersect(log_vars, var_names)) X_model[, lv] <- 10^X_model[, lv]
    output_df <- run_batch(X_model, var_names)
...
for (target_feature in all_targets) {
  ...
  if (nrow(plot_df) > 0) {
    ...
    for (col in plot_facets) {
      # Parse the lower edge of "[lo - hi]" labels (handles scientific notation)
      first_numbers <- as.numeric(sub("^\\[\\s*", "", sapply(strsplit(as.character(clean_df[[col]]), " - "), `[`, 1)))
    ...
    for (lens_name in names(lenses)) {
      ...
      filename <- paste0(plot_prefix, "Lens_", lens_name, "_", target_feature, ".png")
