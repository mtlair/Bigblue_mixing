...
# =============================================================================
# STAGE: pressurized foam-wash column  [PLACEHOLDER for the separate model]
# =============================================================================
# Defining action only: strip entrained gas overhead, collapse/shrink the
# residual bubbles under column pressure (Boyle), set the discharge pressure.
# Optional (off by default so it doesn't pre-empt the real model): a small
# surfactant + fine-solids carry-over into the foam overhead.
foam_wash_column <- function(stream, pars = list()) {
  p <- modifyList(list(
    eta_gas    = 0.75,     # fraction of entrained gas washed out overhead
    P_col_Pa   = 3.0e5,    # column operating pressure (pressurized)
    surf_carry = 0.00,     # fraction of surfactant carried out with the foam
    solid_carry= 0.00,     # fraction of solids floated out with the foam (flotation loss)
    D_b_feed_m = 5.0e-5    # residual bubble size entering the column (pre-collapse)
  ), pars)

  # gas removal ("washing the foam out")
  stream$alpha_g <- max(0, stream$alpha_g * (1 - p$eta_gas))

  # pressurized -> Boyle compression of any residual bubbles + set the bubble
  # field the centrifuge/dryer otherwise have to guess (retires the D_b factor)
  ...
  # optional foam carry-over (default 0): foam is surfactant- and fine-particle-rich
  if (p$surf_carry  > 0) stream$C_surfactant <- stream$C_surfactant * (1 - p$surf_carry)
  if (p$solid_carry > 0) stream$C_solid      <- stream$C_solid      * (1 - p$solid_carry)
