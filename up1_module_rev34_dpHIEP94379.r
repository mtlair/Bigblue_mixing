# ==============================================================================
# INTEGRATED SENSITIVITY ANALYSIS CONTROLLER & PLOTTER (v48 - Full Physics Merge)
# ==============================================================================
...
database_file     <- sprintf("Morris_Omnibus_r%d.rds", trajectories)
...
kinetics_system_rhs <- function(t, state, pars) {
  with(as.list(c(state, pars)), {
    ...
    # --------------------------------------------------------------------------
    Moles_Surf <- C_surfactant / s_pos(MW_surfactant + 1e-6)
    Coverage_Capacity <- Moles_Surf * A_molecule * 1e6 
    ...
    # --------------------------------------------------------------------------
    Rate_floc_A <- (0.01 * (phi_s^2) * epsilon_A * softness_A) / E_stability
    
    denom_break_A <- (1.0 + 0.5 * s_pos(a_trap_A)) * (1.0 + 100.0 * s_pos(t_surf_A)) * softness_A * E_stability
    R_break_raw <- (0.001 * s_pos(D_particle - 0.05) * epsilon_A) / denom_break_A
    R_break_A <- 10.0 * tanh(R_break_raw / 10.0)
    
    Rate_N_A <- Rate_floc_A - (R_break_A * N_A)
    ...
    dC_chem_A_dt <- 0; dn_temp_A_dt <- 0
    dC_chem_B_dt <- 0; dn_temp_B_dt <- 0
...
run_single_simulation <- function(parameters) {
  ...
  phi_template_effective <- ((p$Q_template * p$C_temp_mass) / Q_total) / denom_temp
  
  C_gas_diss_effective <- (p$Q_template * p$C_gas_diss_temp) / Q_total
  
  V_single_template <- (pi / 6) * (p$D_template * 1e-6)^3
  n_temp_initial <- p$chi_npgas * (phi_template_effective / V_single_template)
  ...
  Moles_Surf <- p$C_surfactant / s_pos(p$MW_surfactant + 1e-6)
  Coverage_Capacity <- Moles_Surf * p$A_molecule * 1e6
    ...
    for (lens_name in names(lenses)) {
      ...
      p <- ggplot(lens_df, aes(x = mu_star, y = sigma, color = Zone)) +
        ...
        scale_color_manual(values = c("1. Negligible" = "grey85", "2. Predictable / Additive" = "#27ae60", 
                                      "3. Interacting / Complex" = "#f39c12", "4. Complex Volatility" = "#e74c3c")) +
