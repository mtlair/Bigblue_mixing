# ==============================================================================
# INTEGRATED SENSITIVITY ANALYSIS CONTROLLER & PLOTTER (v51 - Templating Study)
# ==============================================================================
...
database_file     <- sprintf("Morris_Templating_rev37_r%d.rds", trajectories)
plot_prefix       <- "rev37_"
...
run_single_simulation <- function(parameters) {
  ...
  if (is.null(ode_out) || nrow(ode_out) < 2) return(rep(NA, 11))
...
ggsave("rev37_Pendular_Dose_Window.png", p_dose, width = 11, height = 10, dpi = 200)
cat("   > Saved rev37_Pendular_Dose_Window.png\n")
...
for (tg in all_targets) {
  ...
  fn <- paste0("rev37_Morris_", tg, ".png")
...
# --------------------------------------------------------------------------
sink("rev37_templating_summary.md")
cat("# rev37 Templating Morris Summary\n\n")
...
sink()
cat("\n>>> rev37 TEMPLATING STUDY COMPLETE.\n")
